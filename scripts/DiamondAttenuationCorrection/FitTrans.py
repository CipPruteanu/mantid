...
def calcDspacing(a, b, c, alp, bet, gam, h, k, l):
    ...
    oneoverdsq = (1.0-ca**2-cb**2-cg**2+2*ca*cb*cg)**(-1)*((h*sa/a)**2+(k*sb/b)**2+(l*sg/c)**2 +
                                        (2*k*l/(b*c))*(cb*cg-ca)+(2*l*h/(c*a))*(cg*ca-cb)+(2*h*k/(a*b))*(ca*cb-cg))
    ...
    for i in range(n):
        if eqvlab[i] == 0: #then it's not been checked yet, so check it
            ...
            for j in range(i+1, n):  # check remaining indices
                ...
                permcomphkl = list(itt.permutations(comphkl)) #all possible permutations
...
def showx3(x):
    ...
    eqvlab1, neqv1 = findeqvs(hkl1) #% returns array with same dim as input labelling equivs
    ...
    bgd = x[neqv1+neqv2+8-1:neqv1+neqv2+9+2-1]
    pkwid2 = x[neqv1+neqv2+10]
    relsf = x[neqv1+neqv2+11] #% if diamond intensities the same, allow single scale f
...
def SimTransOutput3(name, x):
    ...
    npt = shftlam.shape[0]
    # calculate information for peaks for crystal 1 using hkl,UB1, setang, pkpos
    a, b, c = pkposcalc(hkl1, UB1, setang1)
    pkpars1 = np.column_stack((a, b, c))
    # calculate information for peaks for crystal 2 using hkl,UB1, setang, pkpos
    ...
    for i in range(nref1):
        if pktype == 1:
            ...
            t1 = t1-extScl*pkmult1[int(eqvlab1[i])]*pkcalcint1[i] * (np.exp(-((shftlam-pkpars1[i][0])**2.)/(2*(sig**2))))
    ...
    for i in range(nref2):
        if pktype == 1:
            ...
            t2 = t2-extScl*pkmult2[int(eqvlab2[i])]*pkcalcint2[i] * (np.exp(-(shftlam-pkpars2[i][0])**2./(2*(sig**2))))
...
def FitTrans():
    ...
    fullfilename_ub1 = str(run_number_UB)+'UB1.dat'
    fullfilename_ub2 = str(run_number_UB)+'UB2.dat'
    ...
    a, b, c = pkposcalc(hkl1, UB1, setang1)
    pkpars1 = np.column_stack((a, b, c))
    pkcalcint1 = pkintread(hkl1, (pkpars1[:, 0:3])) #Calculated ref intensities
    pkcalcint1 *= 1e-6
    ...
    a, b, c = pkposcalc(hkl2, UB2, setang2)
    pkpars2 = np.column_stack((a, b, c))
    pkcalcint2 = pkintread(hkl2, (pkpars2[:, 0:3])) #Calculated ref intensities
    pkcalcint2 *= 1e-6
    ...
    lb[neqv1+neqv2+13] = initL2 - delinitL2  # (m) min L2 dist sample to d/stream detector
    ...
    ub[neqv1+neqv2+11] = 1.1  # diamond shouldn't be more than 2 times bigger!
    ub[neqv1+neqv2+12] = 1.1  # max lambda shift
    ub[neqv1+neqv2+13] = initL2 + delinitL2  # (m) max L2 dist sample to d/stream detector
    ...
    max_number_iterations = int(raw_input('Maximum number of iterations for minimization: '))
    ...
    #    x, fval, exitflag, output = fmincon(@SimTrans3,x0,A,b,[],[],Aeq beq lb,ub,[],options)
    ...
    if fxsamediam == 1:
        ...
        x[neqv1+neqv2+11] = 1.0  # label ensuring I know that run did not use fxsamediam
